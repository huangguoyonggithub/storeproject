...
public class WarehouseServiceImpl implements IWarehouseService {
    ...
    @Override
    public Warehouse translateWarehouse(Integer uid, Integer gid) {
        User user = userService.getByUid(uid);
        if (user == null){
            throw new UserNotFoundException("没有用户登录异常");
        }

        Sell sell = sellMapper.selectSellByGid(gid);
        if (sell == null){
            throw new OrderItemNotFoundException("出售商品数据不存在异常");
        }

        Warehouse warehouse = new Warehouse();
        warehouse.setUid(uid);
        //补全基本信息
        warehouse.setGid(sell.getGid());
        warehouse.setPrice(sell.getPrice());
        warehouse.setTitle(sell.getTitle());
        warehouse.setImage(sell.getImage());
        warehouse.setStatus(sell.getStatus());
        ...
        //删除加入仓库的已购商品数据
        sellMapper.deleteSellByGid(gid);
